...
function getData() {
  fetch("https://mindhub-xj03.onrender.com/api/amazing")
    ...
    .then((apiData) => {
      ...
      const pastEvents = events.filter( event => event.date < apiData.currentDate )
      console.log(events);
...
// Search Bar SEGUIR CON LA SEARCHBAR
...
function searchInput(array, text) {
  return array.filter((pastEvent) =>
  pastEvent.name.toLowerCase().includes(text.toLowerCase())
  );
}

eventsSearch.addEventListener("input", () => {
  let arrayFiltered = searchInput(pastEvents, eventsSearch.value);
